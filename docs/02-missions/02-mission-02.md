...
![À compléter avant le 3 novembre 2026 23:59](https://img.shields.io/badge/À%20compléter%20avant%20le-3%20novembre%202026%2023%3A59-red) ![Travail individuel](https://img.shields.io/badge/-Individuel-orange)
...
Pour 10% de la session, transformer votre site HTML de la mission 1 en un site web moderne et attrayant en utilisant CSS, tout en conservant la structure HTML existante.
...
Suite au succès de votre prototype HTML, la direction du refuge est impressionnée et vous mandate maintenant d'ajouter une couche de présentation professionnelle à votre site. Vous devez créer une feuille de style CSS qui transformera votre site HTML brut en un site moderne et engageant, qui donne envie d'adopter!
...
- Utilisez des couleurs qui reflètent l'ambiance chaleureuse et rassurante de votre refuge
...
- Section principale avec grande image de fond (background-image) et texte par-dessus (Communément appelée hero section). Voici un exemple d'une section `hero` avec une image en arrière-plan et du texte par-dessus
  ![Exemple de section hero d'une librairie en ligne: grande image de fond bleue avec le titre du livre « King Kong Theory » et un bouton « View more » par-dessus](./img/hero.jpg)
...
- Pour afficher chaque animal, utilisez un système de "cartes". Voici quelques exemples de carte:
  ![Exemple de carte verticale avec image en haut, titre, court texte descriptif et bouton d'action](./img/card1.png)
  ![Exemple de carte large avec image pleine largeur en haut, titre, texte descriptif et date de mise à jour](./img/card2.png)
  ![Exemple de carte horizontale avec image à gauche et titre accompagné du texte descriptif à droite](./img/card3.png)
- Utilisez `Flexbox` ou `Grid` pour disposer les cartes sous forme de grille
- **Menu de navigation latéral** (gauche ou droite) pour naviguer entre les animaux avec les ancres
- **Utilisation de Flexbox** pour positionner le menu à côté du contenu principal

#### Page des tarifs et étapes d'adoption
- Utilisez un style de couleur alterné pour les lignes du tableau des tarifs. Le style exact est à votre choix, mais vous devez obligatoirement utiliser les pseudo-classes `:nth-child(even)` et `:nth-child(odd)` pour votre tableau.
  :::caution
  ...
  :::
- **Menu de navigation latéral** (aligné à gauche ou à droite du contenu) pour votre table des matières permettant de naviguer entre la section des tarifs et la section des étapes avec les ancres
- Vous devez **utiliser flexbox** pour positionner le menu à côté du contenu principal
- Stylisez votre liste numérotée d'étapes (ex.: numéros personnalisés, espacement, icônes) pour qu'elle soit facile à suivre visuellement
- Le reste est à votre guise pour le style!
...
- Mettez en évidence visuellement les champs obligatoires ou groupez logiquement les champs (ex.: coordonnées vs préférences d'adoption) à l'aide de `div`
...
- FAQ avec `details`/`summary` stylisés (couleurs, bordures, icônes ou symboles pour indiquer l'ouverture/fermeture). Vous devez changer l'apparence pour que cela ressemble à des blocs de FAQ.
- Vous pouvez garder la flèche par défaut du navigateur, mais modifiez le style de la boîte
- Tableau des heures d'ouverture stylisé (bordures, mise en évidence de la journée actuelle ou des jours fermés, par exemple)
...
- Stylisez vos liens `mailto` et `tel` pour qu'ils s'intègrent bien visuellement (ex.: icône ou symbole textuel, couleur cohérente avec le reste du pied de page)
...
- Remis par Léa
- Remis avant le 3 novembre 2026 23:59
...
| Typographie                                                  | 10%        |
| Composants spécifiques: Cartes des animaux                   | 10%        |
| Composants spécifiques: FAQ                                  | 10%        |
| Composants spécifiques: Section hero                         | 10%        |
| Composants spécifiques: Navigation verticale                 | 5%         |
| Composants spécifiques: Formulaire d'adoption                | 10%        |
| Composants spécifiques: Tableau des tarifs alterné           | 5%         |
